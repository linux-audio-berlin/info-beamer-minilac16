...
content = switcher{
    {
        time = 5;
        draw = function()
            light:write(520, 200, "Other rooms", 80, 1, 1, 1, 1)
            ...
            if #all_talks > 0 then
                for idx, talk in ipairs(all_talks) do
                    ...
                    font:write(170, y, talk.place, 50, 1, 1, 1, alpha)
                    font:write(520, y, talk.lines[math.floor((sys.now()/2) % #talk.lines)+1], 50, 1, 1, 1, alpha)
                    y = y + 60
                end
            else
                font:write(520, 330, "No other talks.", 50, 1, 1, 1, 1)
            end
        end
    }, {
        time = 15;
        draw = function()
            if not current_talk then
                light:write(500, 200, "Next talk", 80, 1, 1, 1, 1)
                white:draw(0, 300, WIDTH, 302, 0.6)
                light:write(500, 330, "Nope. That's it.", 50, 1, 1, 1, 1)
            else
                local delta = current_talk.unix - get_now()
                if delta > 0 then
                    light:write(500, 200, "Next talk", 80, 1, 1, 1, 1)
                else
                    light:write(500, 200, "This talk", 80, 1, 1, 1, 1)
                end
                white:draw(0, 300, WIDTH, 302, 0.6)
                ...
                for idx, line in ipairs(current_talk.lines) do
                    ...
                    font:write(500, 330 - 60 + 60 * idx, line, 50, 1, 1, 1, 1)
                end
                for i, speaker in ipairs(current_talk.speakers) do
                    font:write(500, 510 + 50 * i, speaker, 50, 0.2,0.2,0.2,1.0)
                end
            end
            if t then
                local banner = -4 + t
                if banner > 0 and banner < 3.1412 then
                    ...
                    font:write(x, 540, "http://info-beamer.org/", 30, 1, 1, 1, alpha)
                end
            end
         end
    },
...
function node.render()
    ...
    util.draw_correct(tux, 230, 700, 450, 1080)
    bold:write(210, 20, saal, 100, 1, 1, 1, 1)
    bold:write(650, 20, clock.get(), 100, 1, 1, 1, 1)
    bold:write(WIDTH-300, 20, string.format("Day %d", day), 100, 1, 1, 1, 1)
